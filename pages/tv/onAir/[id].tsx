...
import { getOnAir } from "../../../api/tvApi";
...
export default function OnAirTv() {
  ...
  } = useQuery(["OnAirTv", page], () => getOnAir(page));

  {
    onAirTvIsSuccess && console.log("🍋", onAirTvData);
  }
  ...
  return (
    <div className={classes.div}>
      <Text className={classes.title}>On Air TV</Text>

      <Grid grow gutter='md' className={classes.grid}>
        {onAirTvIsSuccess &&
          //@ts-ignore
          onAirTvData.results.map((item) => {
            return (
      ...
      <Box style={{ borderWidth: 10, borderColor: "#FFFFFF" }}>
        ...
        >
          ...
          <Text style={{ fontSize: 14, color: "white", marginRight: 10 }}>
            Page {page} of {onAirTvData?.total_pages}
          </Text>
