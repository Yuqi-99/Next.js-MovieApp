...
export default function AiringTodayTv() {
  ...
  } = useQuery(["airingTodayTv", page], () => getTVTrending(page));

  {
    aTvIsSuccess && console.log("🍋", airingTvData);
  }
  ...
  return (
    <div className={classes.div}>
      <Text className={classes.title}>Airing Today TV</Text>

      <Grid grow gutter='md' className={classes.grid}>
        {aTvIsSuccess &&
          //@ts-ignore
          airingTvData.results.map((item) => {
            return (
      ...
      <Box style={{ borderWidth: 10, borderColor: "#FFFFFF" }}>
        <Flex
          ...
          style={{ marginTop: "5%" }}
        >
          <Link href={`/tv/airingToday/${page - 1}`}>
            {page === 1 ? (
              <Button
                disabled
          ...
          <Text style={{ fontSize: 14, color: "white", marginRight: 10 }}>
            Page {page} of {airingTvData?.total_pages}
          </Text>
