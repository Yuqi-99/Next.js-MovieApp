...
export default function NowPlaying() {
  ...
  } = useQuery(["nowPlaying", page], () => getNowPlaying(page));
  ...
  return (
    <div className={classes.div}>
      <Text className={classes.title}>Now Playing Movies</Text>
      ...
      <Box style={{ borderWidth: 10, borderColor: "#FFFFFF" }}>
        <Flex
          ...
          style={{ marginTop: "5%" }}
        >
          <Link href={`/movies/nowPlaying/${page - 1}`}>
            {page === 1 ? (
              <Button disabled style={{ marginRight: 10, width: 95 }} className={classes.button}>
                Previous
              </Button>
            ) : (
          ...
          <Text style={{ fontSize: 14, color: "white", marginRight: 10 }}>
            Page {page} of {NowPlayingData?.total_pages}
          </Text>
